...
//		db.collection("users").findOne({"credentials.moves.userId":userId}, function(err, user) {
		...
		moves.get("/user/storyline/daily", {
			...
			if (days) {
				async.eachLimit(days, 10, function (day, callback) {
					day.userId = new String(user._id);
					db.collection("moves_summaries").update({date: day.date}, day, {upsert: true}, callback);
				}, callback);
	...
	app.post("/moves", function(request, response) {
		...
		client.publish(process.env.MQTT_BASE_TOPIC, JSON.stringify({job_name:"extract_moves", status:"starting"}));
		...
		pullUser(request.body.userId, oldest, function (err, result) {
			...
			client.publish(process.env.MQTT_BASE_TOPIC, JSON.stringify({job_name:"extract_moves", status:"done"}));
		});
